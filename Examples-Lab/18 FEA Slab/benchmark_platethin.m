...
figure;
%-- Deflexion: W ya viene NEGATIVO (carga descendente). Ploteamos W tal cual
%-- para que la placa se vea cayendo hacia z negativo (convencion estructural).
subplot(2, 2, 1);
colormap('jet');
surf(XX, YY, W);
xlabel('x [m]'); ylabel('y [m]'); zlabel('w [mm] (negativo = hacia abajo)');
title(sprintf('Deflexion BFS — w_{max} = %.3f mm', min(W(:))));
shading('interp');
colorbar;
...
fprintf('=====================================================\n');
fprintf('Match a < 1%% en w, Mx, My (incluyendo signo). Mxy depende de densidad de sampleo.\n');
...
%-- B-matrix 3x16 (B1, B2, B3 = curvaturas κ_xx, κ_yy, κ_xy)
