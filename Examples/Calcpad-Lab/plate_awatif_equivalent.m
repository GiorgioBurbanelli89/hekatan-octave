...
% =============================================================================
%   Caso: placa cuadrilátera con cuatro vértices, sometida a una carga vertical.
...
%   wo80 fork) compilado a triangle.dll (296 KB), expuesto vía P/Invoke en
%   `Calcpad.Core.TriangleInterop.MeshPolygon(points, polygon, maxMeshSize, minAngle)`.
%
%   Ambos producen mallas idénticas: Calcpad Lab + Triangle = paridad exacta
%   con awatif-v2.
% =============================================================================

%% Datos del problema (mismos vértices que awatif)
...
E_x = 100   % Módulo elástico dirección x  [MPa]
E_y = 100   % Módulo elástico dirección y  [MPa]
nu  = 0.3   % Coeficiente de Poisson
t_p = 1     % Espesor                       [m]
G   = 100   % Módulo de corte               [MPa]
q   = -3    % Carga distribuida vertical    [kN/m²]
...
%
% Esta función NO se llama directamente desde MATLAB script aún (pendiente
% de integración runtime). Para invocar desde C# / .NET:
%
%   double[] pts = { 0,0, 15,0, 15,10, 0,5 };
%   int[] polygon = { 0, 1, 2, 3 };
%   var mesh = TriangleInterop.MeshPolygon(pts, polygon, 0.5, 30);
%   // mesh.NumTriangles  → ~270 triángulos (área 112.5 / maxArea 0.5)
%   // mesh.NumPoints     → ~150 puntos
%   // mesh.BoundaryIndices → los puntos en el borde (para BCs)

%% Área del cuadrilátero (verificación analítica)
% Fórmula: A = 0.5 * |x1(y2 − y3) + x2(y3 − y1) + x3(y1 − y2)| (por triángulo)
...
'** Áreas analíticas (cuadrilátero descompuesto en 2 triángulos)
A_T1
A_T2
A_total = A_T1 + A_T2
...
%     → 2 triangles para el cuadrilátero simple SIN refinement.
...
%
% Próximos pasos para FEM completo:
