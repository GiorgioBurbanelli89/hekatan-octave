...
namespace Calcpad.Core.Matlab
{
    public sealed class MatlabPipeline
    {
        ...
        /// <c>do…until</c>, <c>printf</c>, continuación con <c>\</c>). En Calcpad-Octave está
        /// SIEMPRE en true (es el programa dedicado a Octave); se puede forzar a false con
        /// <c>CALCPAD_OCTAVE=0</c> para comparar contra MATLAB estricto.</summary>
        public bool OctaveMode { get; set; }
            = Environment.GetEnvironmentVariable("CALCPAD_OCTAVE") != "0";
